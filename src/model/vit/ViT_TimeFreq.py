...
class ViT_TimeFreq(nn.Module):
    ...
    Args:
        ...
        num_heads: Attention heads. dim_model % num_heads must == 0
        num_layers: Number of TransformerBlocks stacked
        mlp_ratio: MLP hidden dim = dim_model x mlp_ratio
    ...
    def __init__(
        ...
        **kwargs
        ...
        # projecting each patch into a dim_model dimensional vector.
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, 512, dim_model)) # 512 = max capacity
        self.emb_dropout = nn.Dropout(emb_dropout)

        # Transformer encoder (one block = attention + MLP)
        self.blocks = nn.Sequential(*[
            TransformerBlock(dim_model, num_heads, mlp_ratio, dropout)
            for _ in range(num_layers)
        ])

        # Classification head
        self.norm = nn.LayerNorm(dim_model)
        self.head = nn.Linear(dim_model, num_classes)

        self.init_weights()

        n_params = sum(p.numel() for p in self.parameters() if p.requires_grad)
        print(f"[ViT_TimeFreq] {n_params/1e6:.2f}M parameters | "
              f"dim_model={dim_model}, heads={num_heads}, layers={num_layers}, "
              f"patch={patch_size}")
    ...
    def forward(self, x, **kwargs):
        ...
        x = self.patch_embed(x) # (B, num_patches, dim_model)
        N = x.shape[1] # number of patches

        cls = self.cls_token.expand(B, -1, -1) # (B, 1, dim_model)
        x = torch.cat([cls, x], dim=1) # (B, 1+N, dim_model) prepends one CLS token per sample in the batch
        x = x + self.pos_embed[:, :N+1, :] # +1 for the CLS token at position 0
        x = self.emb_dropout(x)
        x = self.blocks(x) # (B, 1+N, dim_model)
